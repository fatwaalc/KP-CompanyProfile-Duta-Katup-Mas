  ...
  <div class="grid grid-cols-[0.8fr_1.2fr] gap-10 items-center bg-gray-50 rounded-xl p-7.5 transition-all duration-500 ease-out hover:bg-white hover:shadow-[0_12px_40px_rgba(0,33,87,0.12)] hover:-translate-y-1.5 animate-slide-in-up md:grid-cols-1 md:gap-7.5 md:p-6 sm:p-4 sm:gap-5">
    ...
    <div class="flex justify-center">
      <div class="relative w-full max-w-96">
        ...
        <!-- Navigation Buttons -->
        <button class="absolute top-1/2 -translate-y-1/2 bg-black bg-opacity-50 text-white border-0 p-3 rounded cursor-pointer transition-all duration-300 hover:bg-opacity-80 z-10 left-3 md:p-2.5" @click="prevSlide">
          <ChevronLeft :size="24" />
        </button>
        <button class="absolute top-1/2 -translate-y-1/2 bg-black bg-opacity-50 text-white border-0 p-3 rounded cursor-pointer transition-all duration-300 hover:bg-opacity-80 z-10 right-3 md:p-2.5" @click="nextSlide">
